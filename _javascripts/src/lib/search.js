...
export default () => {
  ...
  const getExcerpt = (result, query) => {
    ...
    const excerpt = content.slice(searchIndex - 20, (searchIndex + queryLength) + 20)
    ...
    if (query.length > 3) {
      ...
      results.forEach((result) => {
        ...
        <div class="matching-post">
          <a href="${result.querySelector('a').getAttribute('href')}" data-target="${result.querySelector('a').dataset.target}">
